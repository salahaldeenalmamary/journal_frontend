...
import { fetchCompleteReviewAssignment } from '../../store/entities/ReviewerSlice';
...
const CompleteReviewAssignment = () => {
  ...
  useEffect(() => {
    dispatch(fetchCompleteReviewAssignment());
    handleTableContent();
          ...
          <>
            {toggleActions ? (
              <ul className="list-unstyled actions">
                <li>
                <Link to={`/main/reportrecomendition/${record.id}/${record.attachmentId}`}>View Commint</Link>
               
                </li>
                <li>
                <SubmissionView fileId={record.reviewerfileid} text={"View Attatchment"}></SubmissionView>  
                </li>
                <li>
  ...
  const getMenu = (record) => (
    <Menu>
      <Menu.Item key="1">
      <Link to={`/main/reportrecomendition/${record.id}`}>Similar Articles in MEDLINE</Link>
...
export default CompleteReviewAssignment;
